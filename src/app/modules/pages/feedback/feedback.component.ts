...
export class FeedbackComponent {
  ...
  isLoading = false;
  contactUsEmail = 'tritonsupport@kohlerpreferredpartners.com'
  selectedImage = null;
  EmailId: any;
  ...
  ngOnInit(): void {
    const rawEmail = window.sessionStorage.getItem('emailId');
    this.EmailId = decodeURIComponent(rawEmail).replace(/ /g, '+');
  ...
  onFileSelected(event: Event): void {
    ...
    if (input.files && input.files.length > 0) {
      ...
      if (file.type.startsWith('image/')) {
        ...
        this.notification.errorTopRight('Please upload a valid image file');
        input.value = '';
      }
    }
  }
...
onSubmit(event): void {
  ...
  if (this.contactForm.valid) {
    ...
    formData.append('Content', this.contactForm.get('message').value.trim());
    formData.append('files', this.selectedImage);
    
    this.service.contactUs(formData).pipe(
      catchError(error => {
        this.isLoading = false;
        console.log('Error: ', error);
        this.notification.errorTopRight('Something went wrong. Failed to send the message.');
        return of(null);
      })
    ).subscribe(data => {
      this.isLoading = false;
      if (data) {
        if (data.isSuccess) {
          this.notification.successTopRight('Thank you for your Message. We will get back to you as soon as possible');
          ...
          this.notification.errorTopRight(data?.message ?? 'Failed to send the message. Please try again.');
        }
      }
    });
  } else {
    this.notification.errorTopRight('Please fill out the entire form.');
  }
}
