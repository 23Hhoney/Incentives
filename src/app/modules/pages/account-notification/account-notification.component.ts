...
export class AccountNotificationComponent implements OnInit {
  ...
  constructor(
    private _accNotificationService: AccountNotificationService,
    ...
    this.manageWebsiteContent.GetAllCMSMenu(false).subscribe((res: any[]) => {
      ...
      //     this.ApplySearch(searchValue, searchValue.length ? false : true);
      //   });
    }
  ...
  getNotificationList(setNotificationCount: boolean) {
    this.notifications = [];
    this.copiedNotifications = [];
    this.apiRequest.pageIndex = this.currentPage;
    this._accNotificationService.getNotificationList(this.userId, this.apiRequest).subscribe(data => {
      this.notifications = data.results;
      this.copiedNotifications = data.results;
      this.totalRecords = data.totalRecords;
      this.unReadRecords = data.customRecordCount
  ...
  searchNotification(value) {
    this.notifications = []
    if (value === '' || value === null) {
      this.notifications = [...this.copiedNotifications];
    } else {
      this.copiedNotifications.forEach((items => {
        if(items.title.replace(/[ ,?`']/g, '').toString().toLowerCase().includes(value.replace(/[ ,?`']/g, '').toString().toLowerCase())) {
          this.notifications.push(items)
        }
      }))
    }
  }
  ApplySearch(searchValue: string, setNotificationCount: boolean, type, inputElement: HTMLInputElement) {
    if((searchValue === '' && type === 'search') || (searchValue === null && type === 'search') || (searchValue === undefined && type === 'search')){
      return false
    }
    searchValue = type === 'search' ? searchValue.toLowerCase() : '';
    if(type === 'clear') {
      inputElement.value = '';
    }
    searchValue = searchValue.toLowerCase();
    this.searchBarFlag = type === 'search'
    this.apiRequest.search = searchValue;
    this.apiRequest.pageIndex = 1; 
    this.getNotificationList(setNotificationCount);
  }
  ...
  showMessageDialog(dialogName, item) {
    this.msgObj = {
      head: null,
      Sub: null,
      body: null
    }
    this.msgObj.head = item?.title;
    this.msgObj.body = item?.message;
    this.notiModal = this._matDialog.open(dialogName, {panelClass: 'notification-details-container'});
    this.handleReadClick(item)
  }
  ...
  handleReadClick(noti) {
    ...
    if (!noti.isRead) {
      ...
      this._accNotificationService.markRead(request).subscribe(data => {
        if(data.isSuccess) 
          this.getNotificationList(true);
      });
    }
